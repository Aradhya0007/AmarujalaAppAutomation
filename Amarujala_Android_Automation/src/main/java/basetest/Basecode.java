...
public class Basecode {
    ...
    public static String getScreenshot(String testCaseName, AndroidDriver driver) throws IOException {
        File source = driver.getScreenshotAs(OutputType.FILE); // Take screenshot
        String destinationFile = System.getProperty("user.dir") + "/reports/" + testCaseName + ".png"; // Save path
        File finalDestination = new File(destinationFile);
        FileUtils.copyFile(source, finalDestination); // Copy screenshot to destination
        return destinationFile; // Return path to report
    }
